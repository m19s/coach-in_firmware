...
namespace LEDStrip
{
	class Scene6_6Pattern : public BasePattern
	{
	public:
		Scene6_6Pattern()
		    : BasePattern()
		...
		static void configureStrip(LED::Strip *strip, int dissolve_count)
		{
			...
			for (int i = 0; i < strip->numberOfPixels(); i++) {
				if (brightness < 0.5) {
					BasePattern::updatePixel(strip, i, LED::Color::purple(brightness * 100));
				}
				else {
					BasePattern::updatePixel(strip, i, LED::Color::purple((1 - brightness) * 100));
				}
			}
		}
		...
		static std::function<void(LightEmitter *emitter, LED::Strip *strip, EmitCounter counter)> emitHandler()
		{
			return [](LightEmitter *emitter, LED::Strip *strip, EmitCounter counter) {
				Scene6_6Pattern::configureStrip(strip, counter.indexOfDissolveCounter());
			};
		}
	};
}
